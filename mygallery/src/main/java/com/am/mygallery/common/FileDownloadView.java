...
//notice, board 파일 다운 처리용 뷰 클래스
@Component("filedown")//설정 파일에 자동 등록 처리함
public class FileDownloadView extends AbstractView{
	//스프링에서 뷰 클래스 만들려면, 스프링이 제공하는 AbstractView를 상속 받아서 사용 해야함
	
	@Override
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		// 파일 다운로드 처리용 코드 구현
		// 컨트롤러 메소드에서 리턴한 모델의 값(파일객체)을 추출해서 폴더에서 파일을 읽어서 클라이언트로 파일을 출력(전송) 처리함
		...
		//한글 파일명 꺠지지 않도록 인코딩 처리를 위해 파일명만 추출함
		...
		//response로 write() | print() 함 => 스프링에서 제공함
