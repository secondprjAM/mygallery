	...
	<c:forEach items="${ requestScope.list }" var="b">
		<tr>
			...
			<!-- 제목 글자 앞에 댓글과 대댓글 표시 기호 붙임
				들여쓰기 처리 : 원글과 구분지음
			 -->
			<c:if test="${ b.b_lev eq 2 }">&nbsp; &nbsp; ▶ </c:if>
			<c:if test="${ b.b_lev eq 3 }">&nbsp; &nbsp; &nbsp; &nbsp; ▶▶ </c:if>
			...
			<td align="center"><fmt:formatDate value="${ b.b_date }" pattern="yyyy-MM-dd" /></td>
			<td align="center">${ b.b_readcount }</td>
			<td align="center">
				<c:if test="${ !empty b.b_upfile }">◎</c:if>
				<c:if test="${ empty b.b_refile }">&nbsp;</c:if>
			</td>
